...
def py_cpu_nms_poly(dets, thresh):
    ...
    for i in range(len(dets)):
        # 将数据转换为C++的VectorDouble的形式
        # 其中dets[i][0]是单个numpy.float64形式
...
def nmsbynamedict(nameboxdict, nms, thresh):
    ...
    for imgname in nameboxdict:
        ...
        for index in keep:
            # 将其放入同一个图像名字下
            # imgname是key,index是key->的list下的多个检测结果的索引
...
def poly2origpoly(poly, x, y, rate):
    origpoly = []
    for i in range(int(len(poly) / 2)):  # /2是因为x,y都放在一个维度的列表中
...
# 合并检测结果:通过调用不同的IoU计算函数来进行水平框/旋转框的合并
    ...
    for fullname in filelist:
        ...
        with open(fullname, 'r') as f_in:
            ...
            for splitline in splitlines:
                ...
                # 如果这张图片的名字第一次出现在"名字字典中"
                if oriname not in nameboxdict:
                    nameboxdict[oriname] = []  # 建立key->list的映射
                # 向这个图片里添加检测结果
